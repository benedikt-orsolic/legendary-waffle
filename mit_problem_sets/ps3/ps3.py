...
def update_hand(hand, word):
    ...
    for letter in new_word:
        ...
        if letter_count <= 1:
            ...
            new_hand[ letter ] = letter_count - 1
...
def is_valid_word(word, hand, word_list):
    ...
    if '*' in new_word:
        for w in word_list:
            if len(w) == len(new_word):
                is_valid = wild_card_replaceble(new_word, w)
                if is_valid:
                    break
    else:
        is_valid = new_word in word_list
...
def wild_card_replaceble(w, word):
    for i in VOWELS:
        new_w = w[:w.find('*')] + i + w[w.find('*') + 1:]
        if str.lower(new_w) == str.lower(word):
            return True
    return False
#
    ...
    * The hand finishes when there are no more unused letters.
    ...
    while ( letters_in_hand(hand) != 0 ):
        ...
        if is_valid_word(word, hand, word_list):
            word_score = get_word_score(word, letters_in_hand(hand))
            total_score += word_score
            print("\"" + word + "\" earnd " + str(word_score)+ 
...
def play_game(word_list):
    ...
    * Returns the total score for the series of hands
    ...
    for i in range( number_of_hands ):
        ...
        score_for_series += current_hand_score
        print("Total score for this hand: " + str( current_hand_score ))
        
    #End for
    
    print("----------\nTotal score over all hands: " + str(score_for_series))
